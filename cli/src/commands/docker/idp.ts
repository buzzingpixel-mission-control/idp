...
export default class Idp extends Command {
    // Allow variable arguments
    public static strict = false;

    public static summary = `Execute command in the ${chalk.cyan('idp')} image. Empty argument starts a shell session`;
    ...
    public async run (): Promise<void> {
        ...
        this.log(chalk.yellow(
            "You're working inside the 'idp' application container of this project.",
        ));
        ...
        if (this.argv.length) {
            execSync(
                `
                    docker run -it --rm \
                        --entrypoint "" \
                        --name mission-control-idp \
                        -v "${rootDir}:/app" \
                        -w /app \
                        buzzingpixel/mission-control-idp bash -c "${this.argv.join(' ')}";
                `,
                { stdio: 'inherit' },
            );
        ...
        execSync(
            `
                docker run -it --rm \
                    ...
                    -v "${rootDir}:/app" \
                    -w /app \
                    buzzingpixel/mission-control-idp bash;
            `,
            { stdio: 'inherit' },
        );
    }
